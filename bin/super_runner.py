...
data_dir = Path(ModelArgs.data_dir)
...
sequences = [s for s in sequences if len(s) < ModelArgs.length_cutoff]
...
def greedy_cluster(seqs, k, threshold):
    order = sorted(range(len(seqs)), key=lambda i: len(seqs[i]), reverse=True)
    kmers = [kmer_set(s, k) for s in seqs]
    reps, assignment = [], [-1] * len(seqs)
    for idx in order:
        ks = kmers[idx]
        placed = False
        for cid, rep in enumerate(reps):
            denom = min(len(ks), len(kmers[rep]))
            if denom and len(ks & kmers[rep]) / denom >= threshold:
                assignment[idx] = cid
                placed = True
                break
        if not placed:
            ...
            reps.append(idx)
    print(f"[cluster] k-mer greedy -> {len(reps)} clusters")
    return assignment


def clusters_from_mmseqs(seqs, tsv_path):
    rep_of = {}
    ...
    print(f"[cluster] MMseqs2 TSV -> {len(rep_to_cid)} clusters")
    return assignment
...
def cluster_split(assignment, ratios, seed):
    ...
    groups = [cids[:n_tr], cids[n_tr:n_tr + n_va], cids[n_tr + n_va:]]
    out = [[i for c in g for i in members[c]] for g in groups]
    print(f"[split] train {len(out[0])}  val {len(out[1])}  test {len(out[2])}")
    return out


if ModelArgs.mmseqs_tsv and Path(ModelArgs.mmseqs_tsv).exists():
    assignment = clusters_from_mmseqs(sequences, ModelArgs.mmseqs_tsv)
else:
    assignment = greedy_cluster(sequences, ModelArgs.kmer_k, ModelArgs.cluster_threshold)

tr_idx, va_idx, te_idx = cluster_split(assignment, ModelArgs.split_ratios, ModelArgs.split_seed)
...
class Transformer(nn.Module):
    def __init__(self):
        super().__init__()
        self.embed = nn.Embedding(len(aa_vocab), ModelArgs.d_model, padding_idx=pad_idx)
        self.layers = nn.ModuleList([EncoderLayer(ModelArgs.d_model, ModelArgs.num_heads,
                                                  ModelArgs.d_ff, ModelArgs.dropout)
                                     for _ in range(ModelArgs.num_layers)])
        self.final_norm = nn.LayerNorm(ModelArgs.d_model)
        self.fc = nn.Linear(ModelArgs.d_model, num_classes, bias=False)
...
def train(model, train_loader, seed):
    ...
    for epoch in range(ModelArgs.num_epochs):
        ...
        if improved:
            ...
            torch.save(best_state, Path(ModelArgs.out_dir) / f"best_seed{seed}.pth")
        if ema:
            ema.restore(model)
...
torch.save(best[1].state_dict(), Path(ModelArgs.out_dir) / "model_best.pth")
